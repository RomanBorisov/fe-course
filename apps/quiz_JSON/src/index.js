...
function resetState() {
    nextBtn.style.display = 'none';    // nextBtn.classList.add('hidden');
...
async function selectAnswer(e) {
    ...
    const {isCorrect, correctAnswerId} = await quizService.checkAnswer(answerId, questions[currentQuestionIdx].id);
...
async function showScore() {
    resetState();
    const {score} = await quizService.calculateResult(answers);
    answers = [];
